#!/usr/bin/env python3
"""
step9_alert_cloud.py  (v7 — clean signals, no paper trading)
=========================================================
...
def run_prediction(symbol: str, config: dict):
    """Run model prediction for one token. Returns signal dict or None."""
    label     = config['label']
...
def main():
    ...
    SEP = "━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    lines = [f"📊 <b>Signal Check</b>  ⏰ {now_str}", SEP]

    if signals:
        for s in sorted(signals, key=lambda x: x['confidence'], reverse=True):
            lines.append(
                f"{s['tier_emoji']} <b>{s['symbol']} {s['direction']} — {s['tier_label']}</b>\n"
